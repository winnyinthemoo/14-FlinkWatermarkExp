...
$$
D_{avg} = \frac{1}{M} \sum_{i=1}^{M} (T_{output}^{(i)} - T_{window\_ end}^{(i)})
$$

- 其中， $T_{output}$ 为 Flink 算子在处理该窗口时的水位线时间加上水位线延迟时间，代表处理该窗口时收到的最大事件时间， $T_{window\_ end}$ 为该窗口的理论结束时间（事件时间）， $M$ 为触发的窗口总数。水位线的本质是“以延迟换准确”。该指标量化了为了提升准确性，系统必须额外引入多少等待时间。这是评估流处理系统实时性能的关键指标。
